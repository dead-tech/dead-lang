...
UnaryExpression::UnaryExpression(Token::Type unary_operator, std::shared_ptr<Expression> right) noexcept
        : m_operator{unary_operator}, m_right{std::move(right)} {}
...
BinaryExpression::BinaryExpression(std::shared_ptr<Expression> left, Token::Type binary_operator,
                                   std::shared_ptr<Expression> right) noexcept
        : m_left{std::move(left)}, m_operator{binary_operator}, m_right{std::move(right)} {}
...
std::string BinaryExpression::evaluate() const noexcept {
    switch (m_operator) {
        ...
        default: {
            return fmt::format("({} {} {})", m_left->evaluate(), Token::type_to_string(m_operator),
                               m_right->evaluate());
        }
    }
}
...
FunctionCallExpression::FunctionCallExpression(std::string function_name,
        ...
        : m_function_name{std::move(function_name)}, m_arguments{std::move(arguments)} {}
