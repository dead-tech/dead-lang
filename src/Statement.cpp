...
std::string BlockStatement::evaluate() const noexcept {
    return std::accumulate(m_block.begin(), m_block.end(), std::string{},
                           [](const auto &acc, const auto &statement) {
...
ModuleStatement::ModuleStatement(std::string name, BlockStatement functions) noexcept
        : m_name{std::move(name)}, m_functions{std::move(functions)} {}

std::string ModuleStatement::evaluate() const noexcept {
    return m_functions.evaluate();
}

FunctionStatement::FunctionStatement(std::string name, std::string args, std::string return_type,
                                     BlockStatement body) noexcept:
        m_name{std::move(name)}, m_args{std::move(args)}, m_return_type{std::move(return_type)},
...
IfStatement::IfStatement(std::string condition, BlockStatement then_block,
                         BlockStatement else_block) noexcept
        : m_condition{std::move(condition)}, m_then_block{std::move(then_block)}, m_else_block{std::move(else_block)} {}
...
ReturnStatement::ReturnStatement(std::string expression) noexcept: m_expression{std::move(expression)} {}

std::string ReturnStatement::evaluate() const noexcept {
    return "return " + m_expression + ";";
}
...
VariableStatement::VariableStatement(
        ...
        : m_is_mutable{is_mutable}, m_type{type}, m_name{std::move(name)}, m_expression{std::move(expression)} {}
